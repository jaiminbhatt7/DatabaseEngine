...
public:
	...
	int GetSortOrder (OrderMaker &order);
        ...
        //project Schema
        void projectSchema(Schema *s1, int keepMe[], int numAttstoKeep);

        
        //copy and Rename Schema
        void renameSchema(char* newname, Schema *s);

        //join Schema
        void joinSchema(Schema *s1,Schema *s2);

        
        //copy Schema
        void copySchema(Schema *s);

        //add Schema
        void addSchema(char *fpath,char ** attName,char * tablename);

        //check Schema
        int check(char *fpath,char *relName);
        
        //creates an blank Schema
        Schema();
	~Schema ();
