...
int Sort::GetNext (Record &fetchme, CNF &cnf, Record &literal) {
	...
	if(query == NULL){
		...
		*copy = *sortorder;
		...
		/*if(cnforder->numAtts>0)
		{
			for(int i=0;i<sortorder->numAtts;i++)
			{

				cout<<" in loop getnext sort "<< cnforder->numAtts<< " " <<endl;
				int j=0;
				for(j=0;j<cnforder->numAtts;j++)
				{
					cout<<" in loop getnext sort searching for matching att "<< cnforder->whichAtts[j]<< " " <<sortorder->whichAtts[i]<<endl;
					cout<<" in loop getnext sort searching for matching att "<< cnforder->whichTypes[j]<< " " <<sortorder->whichTypes[i]<<endl;
					if((cnforder->whichAtts[j] == sortorder->whichAtts[i]) &&(cnforder->whichTypes[j] == sortorder->whichTypes[i]))
					{
						//do something
						...
						k++;
						cout << " QUERY " << query->whichTypes[i]  << " " << query->numAtts <<endl;
						break;
					}
