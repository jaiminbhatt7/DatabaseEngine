...
void QueryPlan::start()
{
	...
	if(outType == 3)	// None was selected so print query plan
	{
		cout<<"\n--------Printing Tree -------------";
		PrintTree(root);
...
void QueryPlan::executeQueryPlan()
{
	...
	init();
	JoinOrder();
	cout<<" Join Order found !!"<<endl;
	CreateTree();
	cout<<"Tree Created\n";
	DetermineOperation();
}

//for each orlist perform certain operations
void QueryPlan::init()
{
...
void QueryPlan :: JoinOrder()
{
	...
	while(currTable!=NULL)			//al tables in the list are taken and copies created for aliases in stats obj
	{
	...
	while(size == 1 && comboIndex < comboListcopy.size() )		//this is to get combinations limited to size 1, A B C
	{
		...
		if(it2!=SelectionMap.end())					//travarse the orlist related to table in selection map ..generate tree and get estimate
		{										//cost for this is 0
...
Schema * QueryPlan::GetSchema (char * tname)
{
	...
	//cout<<"Table Name passed:"<<tname<<endl;
...
void QueryPlan::PrintTree(BaseNode* node)
{
	...
	case 1: cout<<"SelectFile Operation\n\n";
	cout<<"Relation Name:"<<node->nodeTables<<endl;
	cout<<"Output Pipe ID:"<<node->nodepipeID<<endl;
	...
	case 2: cout<<"SelectPipe Operation\n\n";
	break;

	case 3: cout<<"Project Operation\n\n";
	cout<<"Input Pipe ID:"<<node->left->nodepipeID<<endl;
	cout<<"Output Pipe ID:"<<node->nodepipeID<<endl;
	PrintSchema(node->nodeSchema);
	break;

	case 4: cout<<"Join Operation\n\n";
	cout<<"Input Pipe1 ID:"<<node->left->nodepipeID<<endl;
	cout<<"Input Pipe2 ID:"<<node->right->nodepipeID<<endl;
	cout<<"Output Pipe ID:"<<node->nodepipeID<<endl;
	PrintSchema(node->nodeSchema);
	cout<<"Join CNF:\n";
	node->nodeCnf->Print();
	...
	case 5: cout<<"Duplicate Removal Operation\n\n";
	cout<<"Input Pipe ID:"<<node->left->nodepipeID<<endl;
	cout<<"Output Pipe ID:"<<node->nodepipeID<<endl;
	break;

	case 6: cout<<"Sum Operation\n\n";
	PrintSchema(node->nodeSchema);
	break;

	case 7: cout<<"GroupBy Operation\n\n";
	cout<<"Input Pipe ID:"<<node->left->nodepipeID<<endl;
	cout<<"Output Pipe ID:"<<node->nodepipeID<<endl;
	PrintSchema(node->nodeSchema);
	cout<<"Sort Order\n";
	...
	default: cout<<"Default Operation\n";
	}
	cout<<"\n*****************************************" <<endl;
...
void QueryPlan::PrintSchema(Schema *mySchema)
{
	...
	cout<<"\nOutput Schema : "<<mySchema->GetNumAtts()<<endl;
	Attribute *attrlist=mySchema->GetAtts();
	for(int i=0;i<mySchema->GetNumAtts();i++)
		cout<<"\n\tAtt"<<i<<" : "<<attrlist[i].myType<<" "<<attrlist[i].name<<endl;
...
{
	cout << " Executing Select File "<<endl;
	string fName = node->nodeTables;
	fName.append(".bin");
	cout<<"Bin Table opened : "<<fName<<endl;
